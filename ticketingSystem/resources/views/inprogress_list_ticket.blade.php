    ...
    <div class="row">
        ...
        <div class="col-md-10">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    @foreach ($inprogress_tickets as $inprogress_ticket)
                    <tr>
                        <td>{{ $inprogress_ticket->ticket_number }}</td>
                        <td>{{ $inprogress_ticket->name }}</td>
                        <td>{{ $inprogress_ticket->title }}</td>
                        <td>{{ $inprogress_ticket->description }}</td>
                        <td>{{ $inprogress_ticket->importance }}</td>
                        <td>{{ $inprogress_ticket->date }}</td>
                        <td>{{ $inprogress_ticket->status }}</td>
                        <td>
                            <a href="#" class="btn btn-primary view-btn">Closed</a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
...
<div class="modal fade" id="viewModalinprogress" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <input type = 'submit' class="btn btn-primary" value = "Closed" id="sbmt-closed-tckt">
...
$(document).ready(function(){
  ...
  $('.view-btn').on('click', function(){
    $('#viewModalinprogress').modal('show');
...
$("#sbmt-closed-tckt").click(function(e){
        ...
        $.ajax({
           type:'POST',
           url:'/admin/inprogress_to_closed',
           data:{
                ticket_id:ticket_id,
                name:name,
                title:title,
           ...
           success:function(data){
             $('#viewModalinprogress').modal('hide');
